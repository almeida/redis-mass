...
var RedisCommandParserState = {
    Waitingargstart: function (state, c, eol) {
        if (c !== ' ') {
            if (c === '"') {
    ...
    _WaitingArgumentEnd: function (endToken, state, c, eol) {
        ...
        if (eol || c === endToken) {
            ...
            state.buffer = '';
            state.parser = RedisCommandParserState.Waitingargstart;
        }
    },
...
var RedisCommandParser = {
    parse: function (command) {
        var state = { parser: RedisCommandParserState.Waitingargstart, args: [], buffer: '' },
